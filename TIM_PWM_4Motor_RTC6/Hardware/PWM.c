...
void PWM_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM8, ENABLE); // 开启APB2_TIM8时钟

    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE); // 开启APB2_GPIOA时钟
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE); // 开启APB2_GPIOC时钟

    // GPIOA: TIM1_CH1/CH2/CH3/CH4
    ...
    // GPIOC: TIM8_CH1/CH2/CH3/CH4
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9;
    GPIO_Init(GPIOC, &GPIO_InitStructure);

    TIM_InternalClockConfig(TIM1); // 设置时基单元时钟源为内部时钟
    TIM_InternalClockConfig(TIM8); // 设置时基单元时钟源为内部时钟
    ...
    */
    ...
    TIM_TimeBaseInit(TIM8, &TIM_TimeBaseStructure);  // 初始化时基单元
    ...
    TIM_OC1Init(TIM1, &TIM_OCInitStructure); // TIM1_CH1输出比较初始化
    TIM_OC2Init(TIM1, &TIM_OCInitStructure); // TIM1_CH2输出比较初始化
    TIM_OC3Init(TIM1, &TIM_OCInitStructure); // TIM1_CH3输出比较初始化
    TIM_OC4Init(TIM1, &TIM_OCInitStructure); // TIM1_CH4输出比较初始化

    TIM_OC1Init(TIM8, &TIM_OCInitStructure); // TIM8_CH1输出比较初始化  
    TIM_OC2Init(TIM8, &TIM_OCInitStructure); // TIM8_CH2输出比较初始化
    TIM_OC3Init(TIM8, &TIM_OCInitStructure); // TIM8_CH3输出比较初始化
    TIM_OC4Init(TIM8, &TIM_OCInitStructure); // TIM8_CH4输出比较初始化

    TIM_Cmd(TIM1, ENABLE); // 使能定时器
    TIM_CtrlPWMOutputs(TIM1, ENABLE); // ✅ 高级定时器专用
    TIM_Cmd(TIM8, ENABLE); // 使能定时器
    TIM_CtrlPWMOutputs(TIM8, ENABLE); // ✅ 高级定时器专用
}

void PWM_SetCompare3(uint16_t CompareValue)
{
    TIM_SetCompare1(TIM1, CompareValue); // 设置比较输出1的值
    TIM_SetCompare3(TIM1, CompareValue); // 设置比较输出3的值
    TIM_SetCompare1(TIM8, CompareValue); // 设置比较输出1的值
    TIM_SetCompare3(TIM8, CompareValue); // 设置比较输出3的值
}

void PWM_SetCompare4(uint16_t CompareValue)
{
    TIM_SetCompare2(TIM1, CompareValue); // 设置比较输出2的值
    TIM_SetCompare4(TIM1, CompareValue); // 设置比较输出4的值
    TIM_SetCompare2(TIM8, CompareValue); // 设置比较输出2的值
    TIM_SetCompare4(TIM8, CompareValue); // 设置比较输出4的值
}
