...
{
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM6, ENABLE); // 开启APB1_TIM6时钟

    TIM_InternalClockConfig(TIM6); // 设置时基单元时钟源为内部时钟

    /*
    计数器溢出频率：CK_CNT_OV = CK_CNT / (ARR + 1)
                             = CK_PSC / (PSC + 1) / (ARR + 1)
                    本次示例 = 72000000 / (7200 + 1) / 100
    ...
    */
    ...
    TIM_TimeBaseStructure.TIM_Period = 100 - 1;   // ARR自动重装器的值
                                                    // 单位为计数器个数,范围为0-65535
    TIM_TimeBaseStructure.TIM_Prescaler = 720 - 1; // PSC预分频器的值（10K的频率）/
                                                    // 单位为分频因子，范围为0-65535
    TIM_TimeBaseStructure.TIM_ClockDivision     = TIM_CKD_DIV1;
    TIM_TimeBaseStructure.TIM_CounterMode       = TIM_CounterMode_Up;
    TIM_TimeBaseStructure.TIM_RepetitionCounter = 0; // 重复计数器的值，单位为计数器个数
    TIM_TimeBaseInit(TIM6, &TIM_TimeBaseStructure);  // 初始化时基单元

    TIM_ClearFlag(TIM6, TIM_FLAG_Update); // 清除更新中断标志位

    TIM_ITConfig(TIM6, TIM_IT_Update, ENABLE); // 使能更新中断,开启了更新中断到NVIC的通路

    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2); // 设置NVIC的优先级分组为2

    NVIC_InitTypeDef NVIC_InitStructure;
    NVIC_InitStructure.NVIC_IRQChannel                   = TIM6_IRQn; // 定时器在NVIC中的通道号
    ...
    NVIC_Init(&NVIC_InitStructure);                                   // 使能TIM6的NVIC通路
