...
void PWM_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0; // 设置GPIO_Pin_0为输出模式
    ...
    计数器溢出频率：CK_CNT_OV = CK_CNT / (ARR + 1)
                             = CK_PSC / (PSC + 1) / (ARR + 1)
                    本次示例 = 72000000 / 720 / 100 =1000Hz
                             说明：CK_CNT为系统时钟72MHz，CK_CNT_OV的溢出频率为1000KHz
                                  即计数器每毫秒溢出一次。
    ...
    */
    ...
    TIM_OCInitTypeDef TIM_OCInitStructure;  // 配置输出比较单元1参数
...
{
    TIM_SetCompare1(TIM2, CompareValue); // 设置比较输出1的值
}
