...
void Timer_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_0;// 选择GPIOA的PB0
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU; // 设置PA0为上拉输入
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOA, &GPIO_InitStructure); // 配置PA0

    TIM_ETRClockMode2Config(TIM2, TIM_ExtTRGPSC_OFF, 
                            TIM_ExtTRGPolarity_NonInverted, 0x00); 
                            // 设置时基单元时钟源为外部时钟                                                                                     // 极性为非反转，触发源为任意上升沿                                 
    /*
    计数器溢出频率：CK_CNT_OV = CK_CNT / (ARR + 1)
                             = CK_PSC / (PSC + 1) / (ARR + 1)
                    本次示例 = PA0 脉冲频率 / 1 / 10
                             说明：CK_CNT为ETR外部时钟PA0脉冲频率
                                  即计数器检测到PA0脉冲10次溢出一次。
    ...
    */
    ...
    TIM_TimeBaseStructure.TIM_Period = 10 - 1;   // ARR自动重装器的值
                                                    // 单位为计数器个数,范围为0-65535
...
{
    return TIM_GetCounter(TIM2); // 获取定时器的计数值
